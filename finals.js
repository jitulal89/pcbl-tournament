(() => {
  ...
  async function load(){if(!sb)return;const [tt,pp,mm,mp,gg]=await Promise.all([sb.from('teams').select('*').order('name'),sb.from('players').select('*'),sb.from('matches').select('*').eq('stage','finals').order('final_no'),sb.from('match_players').select('*'),sb.from('games').select('*')]);if(tt.error||pp.error||mm.error||mp.error||gg.error){$('finalProgress').textContent='Unable to load Finals data.';return;}teams=tt.data||[];players=pp.data||[];matches=mm.data||[];matchPlayers=mp.data||[];games=gg.data||[];render();}
