(() => {
  ...
  document.querySelectorAll('a[data-page]').forEach(b=>b.addEventListener('click',e=>{const href=b.getAttribute('href');if(href && href.includes('admin.html?page=')){return;}e.preventDefault();const u=new URL(location.href);u.pathname=u.pathname.replace(/[^/]*$/,'admin.html');u.searchParams.set('page',b.dataset.page);if(b.dataset.page!=='player')u.searchParams.delete('player');location.href=u.toString();})); window.addEventListener('popstate',showRequestedPage);
