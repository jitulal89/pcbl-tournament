(() => {
  ...
  function requestedPage(){const p=new URLSearchParams(location.search).get('page');if(['dashboard','live','playerstats','fixtures','teams','rules','finalformat','player','suggestions','more'].includes(p))return p;const f=location.pathname.split('/').pop().toLowerCase();const map={'admin.html':'dashboard'};return map[f]||'dashboard';}
  function requestedPlayer(){return new URLSearchParams(location.search).get('player')||'';}
  function show(page, updateUrl=true){if(!['dashboard','live','playerstats','fixtures','teams','rules','finalformat','player','suggestions','more'].includes(page)) page='dashboard';document.querySelectorAll('.page').forEach(x=>x.classList.add('hidden'));const el=$(page);if(el)el.classList.remove('hidden');document.querySelectorAll('nav a').forEach(x=>x.classList.toggle('active',x.dataset.page===page));if(updateUrl){const u=new URL(location.href);u.searchParams.set('page',page);if(page!=='player')u.searchParams.delete('player');history.pushState({},'',u);}}
  ...
  function playerHistoryHtml(x){
    return x.appearances.slice().sort((a,b)=>(a.match_no||0)-(b.match_no||0)).map(m=>{
      const ownSide=(m.ap||[]).includes(x.p.id)?'A':'B';
      const oppSide=ownSide==='A'?'B':'A';
      const ownNames=(m[ownSide==='A'?'ap':'bp']||[]).map(id=>player(id)?.name).filter(Boolean);
      const oppNames=(m[oppSide==='A'?'ap':'bp']||[]).map(id=>player(id)?.name).filter(Boolean);
      const w=matchWinner(m), isWin=w===x.t.id, isLoss=w&&w!==x.t.id;
      const type=matchType(m), label=isFinal(m)?finalNoLabel(m):type;
      const ownLine=ownNames.join(' · ')||'—', oppLine=oppNames.join(' · ')||'—';
      return `<button type="button" class="player-history-card ${isWin?'history-win':isLoss?'history-loss':''}" data-admin-history-match="${esc(m.id)}">
        <span class="history-icon">${playerStatsTypeIcon(type)}</span>
        <span class="history-main"><b>${esc(label)}</b><span>${esc(team(m.team_a)?.name||'—')} vs ${esc(team(m.team_b)?.name||'—')}</span><small>${esc(formatTime(m.scheduled_at))} · Court ${esc(m.court||'—')} · Your lineup: ${esc(ownLine)} · Opponent: ${esc(oppLine)}</small></span>
        <span class="history-result ${isWin?'win':isLoss?'loss':''}">${isWin?'WON':isLoss?'LOST':'—'}<strong>${esc(fixtureScore(m))}</strong></span>
      </button>`;
  ...
  function openAdminPlayerStats(id, updateUrl=true){
    ...
    if(updateUrl){location.href='admin.html?page=player&player='+encodeURIComponent(id);return;}
  ...
  async function boot(){
    ...
    if(data?.session){
      ...
      state.tournament=t; $('connection').textContent='SUPABASE'; $('connection').className='badge ok'; hideLogin(); showRequestedPage(); await loadCloud(); showRequestedPage(); window.dispatchEvent(new Event('pcbl-admin-ready')); subscribe(); subscribeViewerPresence();
    }
  }
  ...
  document.querySelectorAll('a[data-page]').forEach(b=>b.addEventListener('click',e=>{
    e.preventDefault();
    const page=b.dataset.page||'dashboard';
    const u=new URL(location.href);
    u.pathname=u.pathname.replace(/[^/]*$/,'admin.html');
    u.searchParams.set('page',page);
    if(page!=='player')u.searchParams.delete('player');
    history.pushState({},'',u);
    show(page,false);
    if(page==='suggestions' && window.renderAdminSuggestions) window.renderAdminSuggestions();
  }));
