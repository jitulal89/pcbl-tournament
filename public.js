  ...
  function teamDetailHtml(t){
    ...
    const fixtures=ms.map(m=>{
      ...
      if(isTriplet(m) && m.status==='done'){
        ...
        return `<div class="triplet-result-card"><div class="triplet-result-head"><div><b>Men's Triplet</b><div class="muted">${esc(formatTime(m.scheduled_at))} · Court ${esc(m.court||'—')}</div></div><span class="status done">✓ COMPLETED</span></div><div class="triplet-result-body"><div class="triplet-result-side ${teamWon?'winner-side':teamLost?'loser-side':''}"><div class="triplet-teamline"><b>${esc(team(isA?m.team_a:m.team_b)?.name||'—')}</b><span class="result-badge ${teamWon?'win-badge':'loss-badge'}">${teamWon?'🏆 WON':'✕ LOST'}</span></div><hr><div class="triplet-pair"><b>PHASE 1</b><span>${esc(isA?phase1A:phase1B)}</span></div><div class="triplet-pair"><b>PHASE 2</b><span>${esc(isA?phase2A:phase2B)}</span></div></div><div class="triplet-score"><strong>${esc(score)}</strong><small>Final Score</small></div><div class="triplet-result-side ${teamLost?'winner-side':teamWon?'loser-side':''}"><div class="triplet-teamline"><b>${esc(opp?.name||'—')}</b><span class="result-badge ${teamLost?'win-badge':'loss-badge'}">${teamLost?'🏆 WON':'✕ LOST'}</span></div><hr><div class="triplet-pair"><b>PHASE 1</b><span>${esc(isA?phase1B:phase1A)}</span></div><div class="triplet-pair"><b>PHASE 2</b><span>${esc(isA?phase2B:phase2A)}</span></div></div></div><div class="triplet-result-foot"><span>Men's Triplet · PCBL 2026</span><span>Winner: ${esc(winName)}</span></div></div>`;
      }
