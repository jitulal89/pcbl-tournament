(() => {
  ...
  function requestedPage(){const p=new URLSearchParams(location.search).get('page');if(['dashboard','live','playerstats','fixtures','teams','rules','finalformat','player'].includes(p))return p;const f=location.pathname.split('/').pop().toLowerCase();const map={'index.html':'dashboard','player-stats.html':'playerstats','fixtures.html':'fixtures','teams.html':'teams','rules_public.html':'rules','final_format.html':'finalformat','player.html':'player'};return map[f]||'dashboard';}
  ...
  function playerStatsData(filters={team:'all',type:'all',stage:'all'}){
    ...
    const rows=teams.flatMap(t=>(t.players||[]).filter(p=>!(/dummy/i.test(String(p.name||'')))).map(p=>{
      ...
      const allAppearances=matches.filter(m=>matchPlayers.some(x=>x.match_id===m.id&&x.player_id===p.id)); return {p,t,played:appearances.length,wins,losses,winPct:appearances.length?Math.round(wins*1000/appearances.length)/10:0,singles:countType("Men's Singles")+countType("Women's Singles"),doubles:countType("Men's Doubles")+countType("Women's Doubles")+countType("Mixed Doubles"),triplet:countType("Men's Triplet"),quad:countType("Men's Quadruple"),appearances,allAppearances};
  ...
  function openPublicPlayerStats(id, updateUrl=true){
    ...
    if(updateUrl){location.href='player.html?player='+encodeURIComponent(id);return;}
  ...
  document.addEventListener('click',e=>{const btn=e.target.closest('[data-team-details]');if(btn)openTeamDetails(btn.dataset.teamDetails);const ps=e.target.closest('[data-public-player-stats]');if(ps){ location.href='player.html?player='+encodeURIComponent(ps.dataset.publicPlayerStats); return; }if(e.target.id==='closeTeamDetail'||e.target.id==='teamDetailModal')$('teamDetailModal').classList.add('hidden');});
})();
