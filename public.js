(() => {
  ...
  // Normalize legacy/alternate match-type values so the public site never
  // renders a Triplet as a generic Doubles match.
  ...
  function winner(m) {
    ...
    let ta=0,tb=0;
    gs.forEach(g => { ta += Number(g.score_a||0); tb += Number(g.score_b||0); });

    // For a completed match, the higher final score is the winner. This is
    // especially important for completed Triplet results such as 30-29.
    if (m.status === 'done') {
      if (ta > tb) return m.team_a;
      if (tb > ta) return m.team_b;
      return null;
    }
  ...
  function render() {
    ...
    $('schedule').innerHTML = matches.slice().sort((a,b)=>new Date(a.scheduled_at||0)-new Date(b.scheduled_at||0)).slice(0,12).map(m=>`<div class="player"><span><b>${esc(formatTime(m.scheduled_at))}</b> · Court ${esc(m.court||'—')}<br>${esc(matchType(m))} · ${esc(team(m.team_a)?.name||'—')} vs ${esc(team(m.team_b)?.name||'—')}</span><span class="status ${esc(m.status)}">${esc(m.status)}</span></div>`).join('') || '<p class="muted">No fixtures yet.</p>';
    ...
    $('fixturesBody').innerHTML = matches.slice().sort((a,b)=>(a.match_no||0)-(b.match_no||0)).map(m=>`<tr><td>${esc(formatTime(m.scheduled_at))}</td><td>${esc(m.court||'—')}</td><td>${esc(matchType(m))}</td><td>${esc(team(m.team_a)?.name||'—')}<br><small>${esc(lineup(m,'A'))}</small></td><td>${esc(team(m.team_b)?.name||'—')}<br><small>${esc(lineup(m,'B'))}</small></td><td>${esc(lineup(m,'A'))} vs ${esc(lineup(m,'B'))}</td><td><span class="status ${esc(m.status)}">${esc(m.status)}</span></td><td><b>${esc(scoreText(m))}</b>${m.status==='done'&&winner(m)?`<br><small>Winner: ${esc(team(winner(m))?.name||'')}</small>`:''}</td></tr>`).join('') || '<tr><td colspan="8" class="muted">No fixtures yet.</td></tr>';
  ...
  function courtHtml(m) {
    ...
    return `<div class="court"><div class="courthead"><div><b>Court ${esc(m.court||'—')}</b><div class="muted">${esc(matchType(m))} · ${esc(formatTime(m.scheduled_at))}</div></div><span class="livepill">● LIVE</span></div><div class="scoreteams"><div class="side"><div class="names"><b>${esc(a?.name||'—')}</b><br><small>${esc(lineup(m,'A'))}</small></div><div class="publicscore">${Number(cur.score_a||0)}</div></div><div class="center"><strong>–</strong><div class="games">Games ${ga} : ${gb}</div></div><div class="side"><div class="names"><b>${esc(b?.name||'—')}</b><br><small>${esc(lineup(m,'B'))}</small></div><div class="publicscore">${Number(cur.score_b||0)}</div></div></div></div>`;
  }
  async function load() {
    if (!sb) return;
