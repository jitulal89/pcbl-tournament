(() => {
  ...
  function playerStatsData(filters={team:'all',type:'all',stage:'all'}){
    ...
    const rows=teams.flatMap(t=>(t.players||[]).map(p=>{
  ...
  function renderPlayerStats(){
    ...
    $('publicPlayerStatsSummary').innerHTML=`<div><span>Players</span><b>${total}</b></div><div><span>Completed appearances</span><b>${played}</b></div><div><span>Player wins</span><b>${wins}</b></div><div><span>Avg. win rate</span><b>${total?Math.round(rows.reduce((n,x)=>n+x.winPct,0)/total):0}%</b></div>`;
    body.innerHTML=rows.map((x,i)=>`<tr class="player-stat-row" data-public-player-stats="${esc(x.p.id)}"><td><b>${i+1}</b></td><td><b>${esc(x.p.name)}</b></td><td>${esc(x.t.name)}</td><td>${x.played}</td><td class="result-win">${x.wins}</td><td class="result-loss">${x.losses}</td><td><b>${x.winPct}%</b></td><td>${x.singles}</td><td>${x.doubles}</td><td>${x.triplet}</td><td>${x.quad}</td></tr>`).join('')||'<tr><td colspan="11" class="muted">No completed player appearances found.</td></tr>';
  }
  function openPublicPlayerStats(id){
    const t=teams.find(t=>(t.players||[]).some(p=>p.id===id)); const p=t?.players.find(p=>p.id===id); if(!p)return;
    const x=playerStatsData({team:t.id,type:'all',stage:'all'}).find(r=>r.p.id===id); if(!x)return;
    $('publicPlayerStatsDetail').classList.remove('hidden'); $('publicPlayerStatsDetail').innerHTML=`<div class="player-detail-head"><div><h3>${esc(p.name)}</h3><p class="muted">${esc(t.name)} · ${x.played} played · ${x.wins} wins · ${x.losses} losses · ${x.winPct}% win rate</p></div><button class="btn small" id="closePublicPlayerStats">Close</button></div><div class="player-history">${x.appearances.slice().sort((a,b)=>(a.match_no||0)-(b.match_no||0)).map(m=>{const w=winner(m),isWin=w===t.id,isLoss=w&&w!==t.id;return `<div class="player-history-row"><span><b>${esc(isFinal(m)?finalLabel(m.final_no):matchType(m))}</b><small>${esc(isFinal(m)?'Finals':'League')} · ${esc(m.status)}</small></span><span>${esc(team(m.team_a)?.name||'—')} vs ${esc(team(m.team_b)?.name||'—')}</span><span class="${isWin?'result-win':isLoss?'result-loss':''}">${isWin?'WON':isLoss?'LOST':'—'}</span><span>${esc(scoreText(m))}</span></div>`}).join('')}</div>`;
    $('closePublicPlayerStats').onclick=()=> $('publicPlayerStatsDetail').classList.add('hidden');
  }
  function render(){
